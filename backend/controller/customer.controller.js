...
export const createCustomer = async (req, res) => {
  try {
    ...
    // Handle bulk upload (array of customers)
    if (Array.isArray(customerData)) {
      ...
      for (const customer of customerData) {
        ...
        const mobile = customer.mobile?.startsWith('91')
          ? customer.mobile
          : `91${customer.mobile}`;
        ...
        if (existingCustomer) {
          ...
          if (dueAmount > 0) {
            ...
            if (customer.gender) existingCustomer.gender = customer.gender;
            if (customer.paymentTerm) existingCustomer.paymentTerm = customer.paymentTerm;
